function extractJSONString(text) {
  ...
  if (endPos === -1) {
    ...
    throw new Error(`"JSON格式不完整或无法找到匹配的结束括号"`);
  }
...
function extractAndParseJSON(text) {
  ...
  try {
    ...
    return JSON.parse(jsonStr);
      ...
      try {
          fixedJsonStr = jsonRepair(jsonStr)
          console.log("JSON修复: ", error, "jsonStr: ", jsonStr, "fixedJsonStr: ", fixedJsonStr);
          return JSON.parse(fixedJsonStr);
      } catch (error) {
          console.error("JSON解析错误: ", error, "jsonStr: ", jsonStr, "fixedJsonStr: ", fixedJsonStr);
